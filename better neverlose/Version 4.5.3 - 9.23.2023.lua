...
nick.Elements = {
    ...
    ["defensive"] = {
        pitch = nick.CreateElements.antiaim.defensive:combo("Pitch", {"Zero", "Up", "Down", "Random", "Jitter", "45 deg"}),
        yaw = nick.CreateElements.antiaim.defensive:combo("Yaw", {"Static", "Random", "Side-Way", "Spin"}),
    ...
    ["debug"] = {
        ...
        window = nick.CreateElements.world.debug:switch("Change Window title"),
...
nick.indicators = function()
    ...
    if nick.Elements.indicator.left:get("Fake Duck") and nick.ref.ragebot.fd:get() then
...
-- nick.window_title = function ()

--     -- This is a basic Windows API code
--     -- ofc, they needed load ffi to use C
--     -- knowing this, you can do some interesting things with the Windows API!
--     -- more info plz go to search from google or bing
...
--     -- no dynamic and dont do it, or else may not restore the original title
...
--     hwnd = user32.FindWindowA(game_window_class, game_window_title)

--     if nick.Elements.debug.window:get() then
--         user32.SetWindowTextA(hwnd, new_title)
--     else
...
nick.update_check = function ()
    ...
    if files.read("check.ini") ~= version then
        print_dev("NEW UPDATED! Your loaded script has been outdated, please go to market get script again or go to github download new version")
        print("NEW UPDATED! Your loaded script has been outdated, please go to market get script again or go to github download new version - https://github.com/xXN1ckWa1k3rXx/cheat_lua/blob/main/better%20neverlose/")
    end
