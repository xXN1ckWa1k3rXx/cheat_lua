...
resolver = {
	...
	run = a(function (self)
        ...
        if nick.items.rage.correction_selection:get() == "Jitter correction" then
...
nick.antiaim = function ()
    ...
    for i, v in ipairs(nick.data.conditions) do
        ...
        if nick.get_condition() == v then 
            ...
            if ui.get(nick.items.antiaim.manual_left) then
                nick.data.aa.__yaw = -90
            elseif ui.get(nick.items.antiaim.manual_back) then
                nick.data.aa.__yaw = 0
            elseif ui.get(nick.items.antiaim.manual_right) then
                nick.data.aa.__yaw = 90
            else
