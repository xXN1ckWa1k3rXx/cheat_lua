--[[
    ...
    Branch: Untested Beta
        - 3/17/2024

    Events sounds code in line 992 
]]
...
ui.new_label("lua", "a", "Discord: xxn1ckwa1k3rxx")
ui.new_button("lua", "a", "Author Telegram", function()
    panorama.open().SteamOverlayAPI.OpenExternalBrowserURL("https://t.me/xXN1ckWa1k3rXx")
end)
ui.new_label("lua", "a", "          ")
...
nick.CffiHelper = {
    PlaySound = (function()
		local PlaySound = vtable_bind("engine.dll", "IEngineSoundClient003", 12, "void*(__thiscall*)(void*, const char*, float, int, int, float)")
...
nick.items = {
    ...
    antiaim = {
        ...
        teleport = ui.new_checkbox("aa", "other", "Teleport Key"),
        teleport_key = ui.new_hotkey("aa", "other", "__Teleport Key", true),
        teleport_overridemode = ui.new_combobox("aa", "other", "Teleport override mode", {"Always on", "On hotkey", "Toggle", "Off hotkey"}),
    },
    fakelag = {
    ...
    visuals = {
        indicators = ui.new_multiselect("visuals", "other esp", "[Better GS] Indicators", {"Lag Compensation", "Extra correction", "AX", "AA Inverter", "Manual Yaw", "Watermark (just a text)"}),
    ...
    misc = {
        events_sounds = ui.new_checkbox("misc", "Miscellaneous", "Events sounds"),
        events_sounds_tip = ui.new_label("misc", "Miscellaneous", "Put files in [csgo folder]/sounds"),
        events_sounds_missedsound = ui.new_textbox("misc", "Miscellaneous", "Missed"),
        events_sounds_missedsound_volume = ui.new_slider("misc", "Miscellaneous", "Missed - Volume",0, 100, 70),
    ...
    for i, v in ipairs (nick.data.conditions) do
        ...
        if nick.get_condition() == v then
            ...
            elseif yaw == "Side-Way" then
                nick.data.defensive.yaw = nick.ticks_switch(math.random(1,3), -85, 85)
...
nick.teleport = function ()
    ...
    if not ui.get(nick.items.antiaim.teleport) then 
        ui.set(nick.ref.rage.double_tap[2], ui.set(nick.items.antiaim.teleport_overridemode))
        ui.set(nick.ref.antiaim.on_shot[2], ui.set(nick.items.antiaim.teleport_overridemode))
    return end

    if ui.get(nick.items.antiaim.teleport_key) then
        ui.set(nick.ref.rage.double_tap[2], "On hotkey")
        ui.set(nick.ref.antiaim.on_shot[2], "On hotkey")
    else
        ui.set(nick.ref.rage.double_tap[2], ui.get(nick.items.antiaim.teleport_overridemode))
        ui.set(nick.ref.antiaim.on_shot[2], ui.get(nick.items.antiaim.teleport_overridemode))
    end
...
-- @region Events Sound [UNTESTED BETA]

----------------------

nick.missed_sound = function (event)
    if ui.get(nick.items.misc.events_sounds) then    
        nick.CffiHelper.PlaySound(ui.get(nick.items.misc.events_sounds_missedsound), ui.get(nick.items.misc.events_sounds_missedsound_volume))
    end
end
...
nick.indicator = function ()
    ...
    for k, v in ipairs (ui.get(nick.items.visuals.indicators)) do

        if v == "Extra correction" then
            if ui.get(nick.items.rage.correction_selection) == "Random body yaw >:)" and ui.get(nick.items.rage.correction) then
        ...
        if v == "AA Inverter" then
            if ui.get(nick.items.antiaim.inverter) then
                renderer.indicator(255, 255, 255, 255, "Inverter")
            end
        end

        if v == "Watermark (just a text)" then
            ...
            renderer.text(10, y - 20, 255, 255, 255, 255, "", 0, ("gamesense.pub - DEBUG Mode | %s - Better GameSense [%s] - %s"):format(panorama_api.MyPersonaAPI.GetName(), __version, time))
        end
    end
...
nick.unload = function ()
    ...
    --[[
        when script shutdown/unload will auto run this function
            this function not support unoverride menu items
    ]]

    cvar.c_mindistance:set_int(130) -- thridperson camera
    cvar.c_maxdistance:set_int(130)

    cvar.fog_override:set_int(0) -- fog

    nick.bloom__shutdown() -- bloom
...
nick.menu__loaded = function()
    ...
    for i, v in ipairs (nick.data.conditions) do
        ...
        ui.set_visible(nick.data.defensive_aa_item[i].__ticks_p, (ui.get(nick.items.antiaim.aa_conditions) == v and ui.get(nick.data.defensive_aa_item[i].def_enabled)  ) and (ui.get(nick.data.defensive_aa_item[i].pitch) == "Jitter") )
        ui.set_visible(nick.data.defensive_aa_item[i].__pitch_1, (ui.get(nick.items.antiaim.aa_conditions) == v and ui.get(nick.data.defensive_aa_item[i].def_enabled)  ) and (ui.get(nick.data.defensive_aa_item[i].pitch) == "Static" or ui.get(nick.data.defensive_aa_item[i].pitch) == "Jitter") )
        ui.set_visible(nick.data.defensive_aa_item[i].__pitch_2, (ui.get(nick.items.antiaim.aa_conditions) == v and ui.get(nick.data.defensive_aa_item[i].def_enabled)  ) and (ui.get(nick.data.defensive_aa_item[i].pitch) == "Jitter") )
        ...
        ui.set_visible(nick.data.defensive_aa_item[i].__ticks_y, (ui.get(nick.items.antiaim.aa_conditions) == v and ui.get(nick.data.defensive_aa_item[i].def_enabled)  ) and (ui.get(nick.data.defensive_aa_item[i].yaw) == "Jitter" or ui.get(nick.data.defensive_aa_item[i].yaw) == "Spin" or ui.get(nick.data.defensive_aa_item[i].yaw) == "Sway") )
        ui.set_visible(nick.data.defensive_aa_item[i].__yaw_1, (ui.get(nick.items.antiaim.aa_conditions) == v and ui.get(nick.data.defensive_aa_item[i].def_enabled)) and (ui.get(nick.data.defensive_aa_item[i].yaw) == "Static" or ui.get(nick.data.defensive_aa_item[i].yaw) == "Jitter" or ui.get(nick.data.defensive_aa_item[i].yaw) == "Spin" or ui.get(nick.data.defensive_aa_item[i].yaw) == "Sway") )
        ui.set_visible(nick.data.defensive_aa_item[i].__yaw_2, (ui.get(nick.items.antiaim.aa_conditions) == v and ui.get(nick.data.defensive_aa_item[i].def_enabled)) and (ui.get(nick.data.defensive_aa_item[i].yaw) == "Jitter" or ui.get(nick.data.defensive_aa_item[i].yaw) == "Spin" or ui.get(nick.data.defensive_aa_item[i].yaw) == "Sway") )
    ...
    ui.set_visible(nick.items.antiaim.teleport_overridemode, ui.get(nick.items.antiaim.teleport))
